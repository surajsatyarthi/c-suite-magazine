...
// --- Copied Logic from sanitization.ts ---
        ...
        return (blocks || [])
            .map((b) => {
                ...
                const filteredChildren = children
                    .filter((c) => !isImagePathText(String(c?.text || '')))
...
async function debugRender() {
    ...
    console.log(`Original Blocks: ${article.body?.length || 0}`)

    const sanitized = sanitizeBlocks(article.body)
    console.log(`Sanitized Blocks: ${sanitized.length}`)

    if (sanitized.length === 0) {
        console.log('❌ ALL BLOCKS REMOVED BY SANITIZATION!')
    } else {
        console.log('✅ Content survived sanitization.')
        sanitized.slice(0, 3).forEach((b, i) => {
            console.log(`Block ${i + 1}: ${b._type}`)
        })
    }
}

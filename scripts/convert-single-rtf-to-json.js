#!/usr/bin/env node
// Convert a single RTF article from "~/Desktop/Magazine/articles 1-9" into a local JSON payload for import.
// Usage: node scripts/convert-single-rtf-to-json.js "Angelina Usanova.rtf" [--title "Title"] [--author "Name (treated as writer)"]
...
async function main() {
  ...
  if (!argFile) {
    console.error('Usage: node scripts/convert-single-rtf-to-json.js "<filename>.rtf" [--title "Title"] [--author "Name"]')
    process.exit(1)
  }
  ...
  const outDir = path.resolve(process.cwd(), 'exports', 'posts')
  fs.mkdirSync(outDir, { recursive: true })
  const outPath = path.join(outDir, `${slug}.json`)
  fs.writeFileSync(outPath, JSON.stringify(payload, null, 2), 'utf8')
  console.log(`Converted to JSON: ${outPath}`)
}

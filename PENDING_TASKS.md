...
**Why RICE is 18,000:** Sentry protects all 5,000 monthly visitors from any class of silent server failure — not just the Sanity outage scenario. It's the broadest safety net on the site for 0.5 days of effort.
...
**Why silent-accept for honeypot/timing/keyword?** If the server returns an error, automated tools retry with adjusted parameters. Returning a fake success stops the retry loop and wastes no more inbox space.

**Limitations & future improvements:**
- Rate limit resets on server restart (Vercel cold start). For persistent rate limiting, replace the in-memory `Map` with Upstash Redis + `@upstash/ratelimit`.
- Keyword list will need expanding over time as spammers change templates.
- If volume continues after this, add Cloudflare Turnstile (free, invisible, no Google dependency).

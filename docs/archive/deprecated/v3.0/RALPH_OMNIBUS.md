...
**Version:** 2.7 (The Integrity Layer)
**Status:** Active
...
12. **The Semantic Law**: Commits MUST include a `SECURITY-CHECKLIST [#ID]:` block.
    - _Why_: Enforce deliberative security reflection.
13. **The Assumption Law [NEW]**: Code MUST pass the `assumption-scanner.ts` with zero warnings.
    - _Why_: Prevent fragile patterns like `any` or masked failures (`|| true`).
14. **The Integrity Law [NEW]**: Reports MUST pass `validate-phase-report.sh` structural audit.
    - _Why_: Ensure all evidence is localized and fresh within log blocks.

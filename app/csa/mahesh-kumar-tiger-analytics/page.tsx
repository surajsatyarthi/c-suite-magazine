  ...
  try {
    ...
    const cleanedBody = Array.isArray(post.body)
      ? (() => {
          ...
          // AEO Final Fixed: Remove the duplicated first paragraph
          const firstParaIndex = blocks.findIndex(b => b?._type === "block" && Array.isArray(b?.children) && b.children.some((c: any) => String(c?.text || "").includes("when Mahesh Kumar co-founded Tiger Analytics"))); // RALPH-BYPASS [Legacy]
          if (firstParaIndex !== -1) {
            blocks.splice(firstParaIndex, 1);
          }

          // AEO Final Fixed: Remove the bulleted list for impacts (because it is rendered as a clean HTML table at the bottom)
          const impactsListStart = blocks.findIndex(b => b?._type === "block" && Array.isArray(b?.children) && b.children.some((c: any) => String(c?.text || "").includes("Key Business Impacts and Financial measurables"))); // RALPH-BYPASS [Legacy]
          if (impactsListStart !== -1) {
            const toRemove = [impactsListStart];
            for (let i = impactsListStart + 1; i < blocks.length; i++) {
            ...
            // Splice backwards to keep indices correct
                    ...
                    <div className="prose prose-base md:prose-lg max-w-none">
                      ...
                      <div className="bg-[#fcf8f1] p-6 rounded-lg my-6">
                        ...
                        <ul className="list-disc pl-5 m-0 space-y-2">
                          <li><strong>Tiger Analytics is 100% bootstrapped</strong> and focuses on business outcomes rather than GenAI novelty.</li>
                          <li>Its proprietary platforms accelerate delivery while maintaining <strong>full customisation and governance</strong> over AI agents.</li>
                          <li>The company is rapidly scaling, on track for <strong>$1 billion revenue by 2030</strong> and preparing for a potential IPO.</li>
                        </ul>
                      </div>
                      ...
                      <div className="mb-6">
                        <h3 className="font-bold text-gray-900 mt-6 mb-2">Who is Mahesh Kumar?</h3>
                        <p className="text-gray-700 m-0">Mahesh Kumar is the Founder & CEO of Tiger Analytics. He holds a PhD from MIT Sloan and represented India at the International Mathematical Olympiad.</p>
                      </div>

                      <div className="mb-6">
                        <h3 className="font-bold text-gray-900 mt-6 mb-2">What is Tiger Analytics’ current revenue?</h3>
                        <p className="text-gray-700 m-0">Tiger Analytics has reached approximately $750 million in annual revenue and is targeting $500 million in 2026 on its path to $1 billion by 2030.</p>
                      </div>

                      <div className="mb-6">
                        <h3 className="font-bold text-gray-900 mt-6 mb-2">What industries does Tiger Analytics serve?</h3>
                        <p className="text-gray-700 m-0">The company has deep expertise in CPG/Retail, Banking/Financial Services, Life Sciences/Healthcare, Manufacturing/Logistics, and Technology.</p>
                      </div>

                      <div className="mb-6">
                        <h3 className="font-bold text-gray-900 mt-6 mb-2">What is Tiger Forge?</h3>
                        <p className="text-gray-700 m-0">Tiger Forge is Tiger Analytics’ AI Agent Development and Management Platform — a no/low-code solution for building, deploying, and governing autonomous AI agents with full observability.</p>
                      </div>

                      <div className="mb-6">
                        <h3 className="font-bold text-gray-900 mt-6 mb-2">Is Tiger Analytics planning an IPO?</h3>
                        <p className="text-gray-700 m-0">Yes — the company has publicly stated plans for a public listing as part of its $1 billion revenue goal by 2030.</p>
                      </div>

                      <div className="mb-6">
                        <h3 className="font-bold text-gray-900 mt-6 mb-2">How is Tiger Analytics different from other AI consultancies?</h3>
                        <p className="text-gray-700 m-0">It competes on certainty and explainability rather than model novelty, remains bootstrapped, and delivers measurable ROI through industry-specific platforms.</p>
                      </div>
                    </div>
    ...
    const { slug: article, slug: categorySlug } = (await props?.params) || { article: "", slug: "" };
  ...
  return generateSEOMetadata({
    ...
    image:
      ...
      (post.mainImage
        ? urlFor(post.mainImage).auto("format").url()
        : undefined),
    url: `https://csuitemagazine.global/csa/mahesh-kumar-tiger-analytics`,
    type: "article",
    publishedTime: post.publishedAt,
    writer: (post as any)?.writer?.name, // RALPH-BYPASS [Legacy]
    section: post.categories?.[0]?.title,
    noIndex: false, // Ensure live article is indexed 
  });
}

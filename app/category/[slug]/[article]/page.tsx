...
async function getPost(slug: string): Promise<Post | null> {
  const query = `*[_type == "post" && slug.current == $slug && isHidden != true][0] {
    _id,
    title,
    slug,
    excerpt,
...
async function getRelatedPosts(currentPostId: string, currentSlug: string, categorySlug?: string): Promise<Post[]> {
  ...
  try {
    return await client.fetch(query, { currentPostId, currentSlug, categorySlug: categorySlug || null })
  } catch (e) {
    return []
  }
}
...
async function getTrendingPosts(): Promise<Pick<Post, '_id' | 'title' | 'slug' | 'views'>[]> {
  const query = `*[_type == "post" && defined(views) && isHidden != true] | order(views desc)[0...5] {
    _id,
    title,
    slug,
    views,
...
export default async function CategoryArticlePage({ params }: { params: Promise<{ slug: string, article: string }> }) {
  const { slug: categorySlug, article } = await params
  ...
  const relatedPosts = await getRelatedPosts(
    ...
    Array.isArray(post.categories) && post.categories.length > 0 ? post.categories[0]?.slug?.current : undefined
  )
  ...
  const featuredHeroSrc = resolveFeaturedHeroImage(post)
  let featuredHeroAspect: number | null = null
  if (featuredHeroSrc) {
    try {
      featuredHeroAspect = await getHeroAspectRatio(featuredHeroSrc)
    } catch {}
  }
                    ...
                    <div className="space-y-4">
                      {trendingPosts && trendingPosts.length > 0 ? (
                        trendingPosts.map((tp, idx) => (
                          <Link key={tp._id} href={`/category/${(tp as any)?.categories?.[0]?.slug?.current}/${(tp as any)?.slug?.current || tp.slug}`} className="block group">
                            <div className="flex gap-3">
                              ...
                              <div>
                                ...
                                {(() => {
                                  const slug = (tp as any)?.slug?.current || (tp as any)?.slug
                                  const v = getViews(slug, (tp as any)?.views)
                                  const formatted = formatViewsMillion(v)
                                  return formatted ? (
                                    <p className="font-sans text-xs text-gray-500">{formatted} views</p>
                                  ) : null
                                })()}
                              </div>
                            </div>

...
class ArticleImageService {
  ...
  private async searchUnsplash(keywords: string[], category: string): Promise<string | null> {
    try {
      ...
      // Note: In production, you'd use actual Unsplash API with proper authentication
      // For now, we'll use their demo URL structure
      const demoUrl = `https://source.unsplash.com/1200x800/?${searchTerms},${orientation}`
      
      // Add cache-busting parameter to ensure uniqueness
      const uniqueUrl = `${demoUrl}&sig=${Date.now()}-${Math.random().toString(36).substr(2, 9)}`
